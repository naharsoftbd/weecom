...
public function updateProduct($user_id,$data){
   ...
   // exit;
    ...
    // set product property values
     ...
     $product = $this->conn->db_query("UPDATE `products` SET `name`= '$data->name',`sku` = $data->sku, `description` = '$data->description', `price` = $data->price, `image` = '', `created_by` = $user_id, `updated_by` = $user_id WHERE `products`.`id` = $data->id;");
